...
export function PaymentModal({ mode, data, theme, dataSource, onClose, onSave }: PaymentModalProps) {
    ...
    useEffect(() => {
        const loadPlans = async () => {
            ...
            try {
                const fetchedPlans = await MasterRecordService.getRecords('plans');
    ...
    // --- SMART SEARCH using Express API ---
    const handleSearchCustomer = async (overrideNumber?: string) => {
        ...
        setIsSearching(true);
        try {
            // Get all customers and find by landline
            const customers = await CustomerService.getCustomers();
            const customer = customers.find((c: any) => c.landline === searchNumber);
            ...
            if (customer) {
                ...
                // Fallback: Try finding in payments
                const payments = await PaymentService.getPayments();
                const payment = payments.find((p: any) => p.landlineNo === searchNumber);
                ...
                if (payment) {
                    const tempCustomer: Customer = {
                        ...
                        status: 'Active' as const, source: payment.source as any,
                    ...
                    if (mode === 'add') {
                        setFormData(prev => ({
                            ...
                            mobileNo: payment.mobileNo || '',
                            email: payment.email || '',
                            rechargePlan: payment.rechargePlan || prev.rechargePlan,
                            source: payment.source || prev.source,
                            billAmount: payment.billAmount?.toString() || prev.billAmount
                        }));
                    }
