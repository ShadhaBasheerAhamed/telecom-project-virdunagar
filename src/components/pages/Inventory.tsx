...
export function Inventory({ theme }: InventoryProps) {
  ...
  const fetchMasterData = async () => {
    const routerMakes = await MasterRecordService.getRecords('router_makes');
    const ontMakes = await MasterRecordService.getRecords('ont_makes');
    const ontTypes = await MasterRecordService.getRecords('ont_types');
    setMasterData({
      routerMake: routerMakes,
      ontMake: ontMakes,
      ontType: ontTypes
    });
  };
  ...
  // --- 2. SALES HISTORY (Placeholder - needs sales API) ---
  useEffect(() => {
    // TODO: Implement sales history API
    setSalesHistory([]);
  }, []);

  // Master data is now fetched in the first useEffect
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editMode && formData.id) {
        await InventoryService.updateItem(formData.id, { ...commonData, ...specificData });
        toast.success("Product updated successfully!");
      } else {
        await InventoryService.addItem({ ...commonData, ...specificData });
  ...
  const handleCheckout = async () => {
    ...
    if (confirm(`Confirm sale for ₹${total.toFixed(2)}?`)) {
      try {
        ...
        // 2. Save Sales Record (TODO: Create sales API)
        // const salesData = {
        //   customerName,
        //   customerPhone,
        //   customerEmail,
        ...
        //   timestamp: Date.now()
        // };
        // await SalesService.createSale(salesData);
