  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (summary) {
          setStats({
            cust: summary.customers?.total || 0,
            leads: summary.leads?.total || 0,
            comp: 0, // TODO: Add complaints count to backend summary
            pay: summary.payments?.count || 0
          });
        }
      } catch (e) {
        console.error(e);
...
const StatCard: React.FC<StatCardProps> = ({ label, value, icon: Icon, color, theme, loading }) => {
  const isDark = theme === 'dark';
  return (
    <div className={`p-4 rounded-xl border flex items-center gap-4 shadow-sm ${isDark ? 'bg-[#242a38] border-gray-700' : 'bg-white border-gray-200'
      }`}>
...
const ReportCard: React.FC<ReportCardProps> = ({ title, description, action, loading, icon: Icon, accentColor, theme }) => {
  const isDark = theme === 'dark';
  return (
    <div className={`relative overflow-hidden rounded-xl border p-6 flex flex-col h-full transition-all hover:shadow-lg ${isDark ? 'bg-[#242a38] border-gray-700' : 'bg-white border-gray-200'
      }`}>
      ...
      <button onClick={action} disabled={loading} className={`w-full py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${loading ? 'bg-gray-500 cursor-not-allowed text-white' : `bg-gradient-to-r from-blue-600 to-blue-500 hover:to-blue-400 text-white shadow-md`
        }`}>
